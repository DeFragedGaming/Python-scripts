...
def filterPacket(dataRecv):
    ...
    if srcAddr == srcIP or dstAddr == dstIP:
        return True
    else:
        return False
    return True
...
def logPacket(packet):
    ...
    with open("packet_log.txt", "a") as logfile:
        logfile.write(f"{datetime.datetime.now()} - {packet}\n")
